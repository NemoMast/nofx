...
const generateLog = (id: number, language: string): LogEntry => {
  ...
  switch (type) {
    ...
    case 'SIGNAL':
      msg =
        language === 'zh'
          ? `美股动量信号已确认（Z 分数 ${Math.random().toFixed(3)}）`
          : `US equities momentum signal confirmed (${Math.random().toFixed(3)} z-score)`
      color = 'text-nofx-gold'
      break
    case 'RISK':
      msg =
        language === 'zh'
          ? `风控检查通过：${pairs[Math.floor(Math.random() * pairs.length)]} 敞口未超限`
          : `Risk check passed: ${pairs[Math.floor(Math.random() * pairs.length)]} exposure within limits`
      color = 'text-nofx-danger'
      break
    case 'MACRO':
      msg =
        language === 'zh'
          ? `宏观数据延迟 < ${Math.floor(Math.random() * 10)} 毫秒`
          : `Macro feed latency < ${Math.floor(Math.random() * 10)}ms`
    ...
    default:
      msg =
        language === 'zh'
          ? '系统优化周期完成，正在分配资源。'
          : 'System optimization cycle complete. Allocating resources.'
        ...
        <div className="hidden md:flex items-center gap-6 text-nofx-text-muted border-r border-[rgba(26,24,19,0.14)] pr-6 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-nofx-text">
              {language === 'zh' ? '行情连接：稳定' : 'WS_CONN: STABLE'}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-nofx-gold">
              {language === 'zh' ? '每秒处理量：48,291' : 'TPS: 48,291'}
            </span>
          </div>
        </div>

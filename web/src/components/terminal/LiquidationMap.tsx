  ...
  return (
    <div style={{ fontFamily: 'var(--tm-mono)' }}>
      ...
      >
        ...
        >
          {view.rows.length ? ui('● live') : isLoading ? ui('○ sync') : '○ —'}
        </span>
      </div>
      ...
      >
        <Swatch c={C_LONG_COST} label={ui('Long cost')} />
        <Swatch c={C_SHORT_COST} label={ui('Short cost')} />
        <Swatch c={C_LONG_LIQ} label={ui('Long liq')} />
        <Swatch c={C_SHORT_LIQ} label={ui('Short liq')} />
      </div>
      ...
      >
        {hv ? (
          ...
          <span className="tm-sc">
            ...
            · {view.costAddrs.toLocaleString()} {ui('positions /')}{' '}
            {view.liqAddrs.toLocaleString()} {ui('liq levels')}
          </span>
        )}
      </div>
      ...
      {error && !view.rows.length ? (
        <div className="tm-sc" style={{ padding: '16px 0' }}>
          {ui('No cost/liq heatmap for')} {view.dispSymbol}{' '}
          {ui('(crypto / main-dex markets have none).')}
        </div>
      ) : !view.rows.length ? (
        <div className="tm-sc" style={{ padding: '16px 0' }}>
          {ui('Loading cost/liquidation map…')}
        </div>
      ) : (
        <div>

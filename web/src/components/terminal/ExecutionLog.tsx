  ...
  return (
    <div style={{ fontFamily: 'var(--tm-mono)' }}>
      ...
      >
        ...
        >
          {cycles.length ? ui('{count} cyc', { count: cycles.length }) : '—'}
        </span>
      </div>
      <div className="tm-sc" style={{ fontSize: 9, marginBottom: 5 }}>
        {ui('Execution log · AI decisions & fills per cycle')}
      </div>
      ...
      >
        <Legend glyph="✓" c="var(--tm-up)" label={ui('ok')} />
        <Legend glyph="⚠" c={C_AMBER} label={ui('throttle')} />
        <Legend glyph="❌" c="var(--tm-dn)" label={ui('risk')} />
      </div>
    ...
    <div style={{ borderBottom: '1px solid var(--tm-hair)', padding: '5px 0' }}>
      ...
      >
        ...
        <span style={{ marginLeft: 'auto', color: 'var(--tm-muted)' }}>
          {count === 0
            ? ui('no action')
            : ui(count > 1 ? '{count} actions' : '{count} action', { count })}
        </span>
        {!record.success ? (

...
export function TerminalDashboard({
  ...
  async function closePositionRow(symbol: string, side: 'LONG' | 'SHORT') {
    ...
    const ok = await confirmToast(
      ui('Market-close {symbol} {side}?', { symbol, side: ui(side) }),
      {
        title: ui('Close position'),
        okText: ui('close'),
    ...
    try {
      ...
      notify.success(ui('{symbol} {side} closed', { symbol, side: ui(side) }))
      await Promise.all([
        mutate(`positions-${traderId}`),
        mutate(`account-${traderId}`),
      ])
    } catch (err) {
      notify.error(err instanceof Error ? err.message : ui('Close failed'))
    } finally {
      setClosing(null)
    }
  }
  ...
  async function closeAllPositions(open: Position[]) {
    ...
    const ok = await confirmToast(
      ui('Market-close ALL {count} open positions?', { count: open.length }),
      {
        title: ui('Flatten book'),
        okText: ui('Close all'),
        cancelText: ui('Cancel'),
      }
    )
    ...
    if (failed === 0) notify.success(ui('All positions closed'))
    else
      notify.error(
        ui('{failed}/{count} closes failed', { failed, count: open.length })
      )
    setClosing(null)
  }
  ...
  return (
    <div className="nofx-terminal" style={{ minHeight: '100vh', padding: 0 }}>
      ...
      <button
        type="button"
        onClick={() => setDemo((v) => !v)}
        aria-label={ui('toggle presentation mode')}
        style={{
          position: 'fixed',
          right: 10,
          bottom: 10,
      ...
      >
          ...
          (status.safe_mode ||
            ...
            >
              <span style={{ fontWeight: 600 }}>
                {status.ai_wallet_status === 'empty'
                  ? ui('AI fee wallet is out of USDC — decisions are failing.')
                  : status.ai_wallet_status === 'low'
                    ? ui(
        ...
        >
          ...
          <span>
            <span className="tm-sc">{ui('model')} </span>
            {(() => {
          ...
          <span>
            <span className="tm-sc">{ui('strategy')} </span>
            {config?.strategy_name || selectedTrader?.strategy_name || '—'}
          </span>
          <span>
            <span className="tm-sc">{ui('lev')} </span>
            {config?.btc_eth_leverage ?? '—'}× /{' '}
            {config?.altcoin_leverage ?? '—'}×
          </span>
          <span>
            <span className="tm-sc">{ui('scan')} </span>
            {scanMin}
            {ui('m')}
          </span>
          <span>
            <span className="tm-sc">{ui('universe')} </span>
            {candidateCoins.length}
          </span>
          <span>
            <span className="tm-sc">{ui('positions')} </span>
            {positions?.length ?? 0}
          </span>
          <span style={{ marginLeft: 'auto' }}>
            <span className="tm-sc">{ui('next cycle')} </span>
            {countdown}
          </span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)' }}>
          {[
            ...
            {
              l: ui('Total P/L · incl. unrealized'),
              v: `${fmtUsd(pnl, true)} (${fmtPct(pnlPct)})`,
              c: up ? 'var(--tm-up)' : 'var(--tm-dn)',
            },
            {
              l: ui('Realized P/L · closed trades'),
          ...
          <>
            ...
            >
              ...
              <span className="tm-sc tm-up">
                {ui('win')} {fullStats.win_trades} (
                {fullStats.win_rate.toFixed(1)}%)
              </span>
              <span className="tm-sc tm-dn">
                {ui('loss')} {fullStats.loss_trades}
              </span>
        ...
        >
          ...
          <div style={sc}>
            ...
            >
              ...
              <span className="tm-sc">{ui('Current positions · live')}</span>
              <span className="tm-sc" style={{ marginLeft: 'auto' }}>
                {positions?.length ?? 0} {ui('open')}
              </span>
              {traderId && !on && positions && positions.length > 0 && (
                ...
                >
                  {closing === '__all__' ? ui('closing…') : ui('close all')}
                </button>
              )}
            </div>
              ...
              >
                <thead>
                  <tr className="tm-sc" style={{ fontSize: 9 }}>
                    <td style={{ padding: '0 0 3px' }}>{ui('symbol')}</td>
                    <td style={{ padding: '0 0 3px' }}>{ui('side·lev')}</td>
                        ...
                        {traderId && !on && (
                          ...
                          >
                            <button
                              ...
                              disabled={closing !== null}
                              title={ui('Close {symbol}', { symbol: p.symbol })}
                              className="tm-mono"
                              style={{
                                background: 'transparent',
                                border: '1px solid var(--tm-dn)',
            ...
            >
              ...
              <span className="tm-sc">
                {ui('Recent closes · symbol/side/hold/pnl')}
              </span>
            </div>
            {recentTrades.length > 0 ? (
              ...
              >
                <thead>
                  <tr className="tm-sc" style={{ fontSize: 9 }}>
                    <td style={{ padding: '0 0 3px' }}>{ui('symbol')}</td>
                    <td style={{ padding: '0 0 3px' }}>{ui('side')}</td>
                    <td style={{ padding: '0 0 3px', textAlign: 'right' }}>
                      {ui('hold')}
                    </td>
                    <td style={{ padding: '0 0 3px' }}> {ui('closed')}</td>
                    <td style={{ padding: '0 0 3px', textAlign: 'right' }}>
                      {ui('PnL')}
                    </td>
                  </tr>
        ...
        >
          <div style={{ ...sc, borderRight: cellBorder }}>
            ...
            >
              <span className="tm-px" style={{ fontSize: 12 }}>
                {ui('Market net inflow')}
              </span>
              <span className="tm-sc">
                {ui('Market net inflow ·')} {flow?.data?.window || '1h'} ·
                Vergex
              </span>
              <span className="tm-sc" style={{ marginLeft: 'auto' }}>
                {flowItems.length} {ui('markets')}
              </span>
            </div>
          ...
          <div style={{ ...sc, borderRight: cellBorder }}>
            ...
            >
              ...
              <span className="tm-sc">
                {ui('By-symbol history · trades/win/pnl')}
              </span>
            </div>
            {symbolStats.length > 0 ? (
                ...
                <div key={s.symbol} style={{ marginBottom: 7 }}>
                  ...
                  >
                    ...
                    <span className="tm-sc" style={{ marginLeft: 8 }}>
                      {s.total_trades} {ui('trades ·')} {s.win_rate.toFixed(0)}
                      {ui('% win')}
                    </span>
                    <span
              ...
              <div className="tm-sc">{ui('No symbol history.')}</div>
            )}
          </div>
          <div style={sc}>
            <div
            ...
            >
              ...
              <span className="tm-sc">
                {ui('Net by hold time & side · after fees')}
              </span>
            </div>

...
export function AdvancedChart({
  ...
  useEffect(() => {
    ...
    const loadData = async (isRefresh = false) => {
      ...
      try {
            ...
            ordersByCandle.forEach((counts, candleTime) => {
              ...
              if (counts.buys > 0) {
                markers.push({
                  ...
                  text: counts.buys > 1 ? `${ui('B')}${counts.buys}` : ui('B'),
                  size: 1,
                })
              }
              ...
              if (counts.sells > 0) {
                markers.push({
                  ...
                  shape: 'circle' as const,
                  text:
                    counts.sells > 1 ? `${ui('S')}${counts.sells}` : ui('S'),
                  size: 1,
                })
              }
            })
        ...
        setError(ui(err.message || 'Failed to load chart data'))
        setLoading(false)
      }
    }
  ...
  }, [symbol, interval, traderID, exchange, ui])
